#include "../include/advancedQFcpp_bits/matrix.hpp"
#include "../include/advancedQFcpp_bits/fx1.hpp"

#include <algorithm>
#include <cmath>

result_data FX1::evaluate_data_and_premium() const
{

    double dtau, alpha, k;

    std::vector<double> t, tau, S, x;

    matrix<double> u, v;

    matrix_resize(u, N, M);
    matrix_resize(v, N, M);

    t.resize(M);
    tau.resize(M);

    S.resize(N);
    x.resize(N);

    dtau = dt * (0.5 * sigma * sigma);
    alpha = dtau / (dx * dx);
    k = r / (0.5 * sigma * sigma);

    double xmin = -1;
    double xmax = 1;

    for (int i = 0; i < N; i++)
    {
        x[i] = xmin + i * dx;
        S[i] = K * exp(x[i]);
    }

    for (int j = 0; j < M; j++)
    {
        t[j] = j * dt;
        tau[j] = (T - t[j]) / (0.5 * sigma * sigma);
    }

    for (int i = 0; i < N; i++)
    {
        u[i][0] = std::max(exp(0.5 * (k + 1) * x[i]) - exp(0.5 * (k - 1) * x[i]), 0.);
    }

    for (int j = 1; j < M; j++)
    {
        u[0][j] = 0.;
        if (barrier)
        {
            u[N - 1][j] = 0.;
        }
        else
        {
            u[N - 1][j] = u[N - 1][0];
        }
    }

    for (int j = 0; j < M - 1; j++)
    {

        for (int i = 1; i < N - 1; i++)
        {
            u[i][j + 1] = alpha * u[i + 1][j] + (1 - 2 * alpha) * u[i][j] + alpha * u[i - 1][j];
        }
    }

    for (int j = 0; j < M; j++)
    {
        for (int i = 0; i < N; i++)
        {
            v[i][j] = pow(K, (0.5 * (1 + k))) * pow(S[i], (0.5 * (1 - k))) * exp(1. / 8. * (k + 1) * sigma * sigma * (T - t[i])) * u[i][j];
        }
    }

    result_data result(alpha, dtau, k, x, S, t, tau, u, v);

    return result;
}